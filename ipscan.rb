...
sleep (0.1)
puts "#{$r}[ 15_Reverseiplookup	]#{$w}"
sleep (0.1)
puts "#{$r}[#{$y} 16_Port_Scan		#{$r}]#{$w}"
sleep (0.1)
puts "#{$r}^^^^^^^^^^^^^^^^^^^^^^^^^#{$w}"
sleep (0.3)
print "#{$y}Enter any number = #{$g}"
  ...
  ans =gets.chomp
	...
	puts "Version = 0.4 "
	when '15'
	puts "Enter hostname or ip ?"
	revip = gets.chomp
	revse =	Net::HTTP.get(URI("https://api.hackertarget.com/reverseiplookup/?q=#{revip}\n\n\n"))
	puts revse
	when '16'
	system"clear"
puts "#{$r}[#{$y} Portscan  #{$r}]#{$w}"
	system"pkg install nmap -y"
	system"clear"
	print "#{$c}started-Nmap"
	sleep (0.1)
	print "#{$r}."
	sleep (0.1)
	print "."
	...
	sleep (0.1)
	puts "."
	puts "#{$m}Enter hostname or ip ? #{$c}"
	ipss = gets.chomp
	system"nmap -r -v #{ipss}"
